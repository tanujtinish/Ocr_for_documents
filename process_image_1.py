...
def dilate(ary, N, iterations):
    ...
    print("kernel",kernel,N)
    dilated_image = cv2.dilate(ary / 255, kernel, iterations=iterations)

    kernel = np.zeros((N, N), dtype=np.uint8)
    kernel[:, (N - 1) // 2] = 1  # Bug solved with // (integer division)
    print("kernel2",kernel,N)
...
def find_components(edges, max_components=16):
    ...
    while count > 16:
        ...
        contours= cv2.findContours(dilated_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def process_image_1(path):
    path=str(path)
    out_path = path.replace("."+path.split('.')[2], '_crop.'+path.split('.')[2])
